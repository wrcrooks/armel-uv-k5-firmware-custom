...
### `ENABLE_FMRADIO_MINIMIZED` (default: 0)
Only affects anything when `ENABLE_FMRADIO=1`. Selects between two complete implementations of `app/fm.c`/`ui/fmradio.c`/the FM half of `app/action.c`'s `ACTION_Scan_FM`, both built from the same source (behind `#ifdef ENABLE_FMRADIO_MINIMIZED`/`#else`), not two separate files: with it **on**, FM radio drops the 20-channel-preset memory, MR (memory recall) mode, auto-scan-to-fill-memory, and direct keypad frequency entry, keeping only step-tuning, band cycling, and seek-scan — saving ~1.5KB over the full implementation. With it **off** (the default), FM radio is the original full-featured implementation. `gFM_Channels`/`FM_IsMrMode`/`FM_SelectedChannel` stay in the EEPROM layout either way (`settings.c` loads/saves them unconditionally) so switching this flag doesn't touch EEPROM compatibility, even though the minimized build never acts on that data. Useful when other enabled flags leave too little flash headroom for the full FM implementation (see the flash-cost table below).
...
**Baseline** at time of measurement (upstream `main`'s own defaults,
`EDITION_STRING=Custom TARGET=f4hwn`): `.text` 52,960 + `.data` 16 = 52,976
bytes, against a 61,440-byte (60KiB) `FLASH` region — **8,464 bytes of
headroom**. That headroom is large enough that every flag below linked
successfully in both states, so all figures are exact, direct measurements
rather than overflow-based estimates.
...
a different one (e.g. once several other flags are also enabled and headroom
shrinks). Re-run the script after changing your own baseline configuration if
you need numbers accurate to it.

This sweep caught **four rows** hitting genuine, pre-existing compile errors
unrelated to flash budget — real latent bugs (undeclared identifiers, and one
unbalanced `#ifdef`/`#else`/`#endif` spanning an `if`/`else` in a way that
left a stray unmatched brace) in code paths that only compile under specific
flag combinations: `ENABLE_VOICE`, `ENABLE_BIG_FREQ`, and `ENABLE_SCAN_RANGES`
fail to build in their non-default state, as does `ENABLE_FEAT_F4HWN` (the
non-F4HWN "egzumer" build, a separate `driver/backlight.c` issue). These are
all marked N/A below and are out of scope for this PR — see the companion
"flag-combination bugfixes" PR for `ENABLE_VOICE`/`ENABLE_BIG_FREQ`/
`ENABLE_SCAN_RANGES`.

| Flag | Default | Cost when ON (flash bytes) | Notes |
|---|---|---|---|
| `ENABLE_FMRADIO` | 0 | +4536 | measured directly |
| `ENABLE_FMRADIO_MINIMIZED` | 0 | -1528 | not yet in `main` - introduced by a companion PR; measured directly against `ENABLE_FMRADIO=1` (only has an effect when FM radio is enabled) |
| `ENABLE_UART` | 1 | +1076 | measured directly |
| `ENABLE_AIRCOPY` | 0 | +1740 | measured directly |
| `ENABLE_NOAA` | 0 | +1500 | measured directly |
| `ENABLE_VOICE` | 0 | N/A | fails to build when enabled - pre-existing bug, see above |
| `ENABLE_VOX` | 1 | +764 | measured directly |
| `ENABLE_ALARM` | 0 | +768 | measured directly |
| `ENABLE_TX1750` | 1 | +196 | measured directly |
| `ENABLE_PWRON_PASSWORD` | 0 | +560 | measured directly |
| `ENABLE_DTMF_CALLING` | 0 | +3540 | measured directly |
| `ENABLE_FLASHLIGHT` | 1 | +64 | measured directly |
| `ENABLE_SPECTRUM` | 0 | +6816 | measured directly |
| `ENABLE_BIG_FREQ` | 1 | N/A | fails to build when disabled - pre-existing bug, see above |
| `ENABLE_SMALL_BOLD` | 1 | +604 | measured directly |
| `ENABLE_CUSTOM_MENU_LAYOUT` | 1 | -80 | measured directly |
| `ENABLE_KEEP_MEM_NAME` | 1 | +12 | measured directly |
| `ENABLE_WIDE_RX` | 1 | +76 | measured directly |
| `ENABLE_TX_WHEN_AM` | 0 | -12 | measured directly |
| `ENABLE_F_CAL_MENU` | 0 | +184 | measured directly |
...
| `ENABLE_SHOW_CHARGE_LEVEL` | 0 | +172 | measured directly |
| `ENABLE_REVERSE_BAT_SYMBOL` | 0 | +0 | measured directly |
| `ENABLE_NO_CODE_SCAN_TIMEOUT` | 1 | -24 | measured directly |
| `ENABLE_AM_FIX` | 1 | +552 | measured directly |
| `ENABLE_SQUELCH_MORE_SENSITIVE` | 1 | +92 | measured directly |
| `ENABLE_FASTER_CHANNEL_SCAN` | 1 | +0 | measured directly |
| `ENABLE_RSSI_BAR` | 1 | +512 | measured directly |
...
| `ENABLE_BYP_RAW_DEMODULATORS` | 0 | +16 | measured directly |
| `ENABLE_BLMIN_TMP_OFF` | 0 | +32 | measured directly |
| `ENABLE_SCAN_RANGES` | 1 | N/A | fails to build when disabled - pre-existing bug, see above |
| `ENABLE_REGA` | 0 | +704 | measured directly |
| `ENABLE_EXTRA_UART_CMD` | 0 | +196 | measured directly |
| `ENABLE_FEAT_F4HWN` | 1 | N/A | fails to build when disabled - pre-existing bug, see above |
| `ENABLE_FEAT_F4HWN_GAME` | 0 | +2516 | measured directly |
| `ENABLE_FEAT_F4HWN_SCREENSHOT` | 0 | +452 | measured directly |
| `ENABLE_FEAT_F4HWN_SPECTRUM` | 1 | +0 | measured directly |
| `ENABLE_FEAT_F4HWN_RX_TX_TIMER` | 1 | +196 | measured directly |
| `ENABLE_FEAT_F4HWN_CHARGING_C` | 0 | +36 | measured directly |
| `ENABLE_FEAT_F4HWN_SLEEP` | 1 | +500 | measured directly |
| `ENABLE_FEAT_F4HWN_RESUME_STATE` | 1 | +292 | measured directly |
| `ENABLE_FEAT_F4HWN_NARROWER` | 1 | +188 | measured directly |
| `ENABLE_FEAT_F4HWN_INV` | 1 | +16 | measured directly |
| `ENABLE_FEAT_F4HWN_CTR` | 1 | +64 | measured directly |
| `ENABLE_FEAT_F4HWN_RESCUE_OPS` | 0 | +648 | measured directly |
| `ENABLE_FEAT_F4HWN_FLASHLIGHT_SOS` | 0 | +196 | not yet in `main` - introduced by a companion PR; measured directly |
| `ENABLE_FEAT_F4HWN_VOL` | 0 | +164 | measured directly |
...
| `ENABLE_AM_FIX_SHOW_DATA` | 0 | +332 | measured directly |
| `ENABLE_AGC_SHOW_DATA` | 0 | -36 | measured directly |
| `ENABLE_UART_RW_BK_REGS` | 0 | +68 | measured directly |
...
| `ENABLE_OVERLAY` | 0 | +644 | measured directly |
| `ENABLE_LTO` | 1 | -4500 | measured directly |
| `ENABLE_EXPERIMENTAL_CLFAGS` | 1 | -12 | measured directly |

A few standouts: `ENABLE_LTO` is by far the single biggest lever on this list
(-4.5KB when disabled) and costs nothing feature-wise to keep on.
`ENABLE_SPECTRUM` (+6.8KB) and `ENABLE_FMRADIO` (+4.5KB) are the priciest
actual features. Several flags measure `+0` exactly —
`ENABLE_CTCSS_TAIL_PHASE_SHIFT`, `ENABLE_BOOT_BEEPS`,
`ENABLE_REVERSE_BAT_SYMBOL`, `ENABLE_FASTER_CHANNEL_SCAN`,
`ENABLE_REDUCE_LOW_MID_TX_POWER`, `ENABLE_FEAT_F4HWN_SPECTRUM` — these swap
constants/behavior rather than adding code, so they're effectively free
either way.
